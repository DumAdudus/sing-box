...
func parseISCDhcpd(file *os.File, ipToMAC map[netip.Addr]net.HardwareAddr, ipToHostname map[netip.Addr]string, macToHostname map[string]string) {
	...
	for scanner.Scan() {
		...
		if after, ok := strings.CutPrefix(line, "hardware ethernet "); ok {
			macString := strings.TrimSuffix(after, ";")
			parsed, macErr := net.ParseMAC(macString)
			if macErr == nil {
				currentMAC = parsed
			}
		} else if after, ok := strings.CutPrefix(line, "client-hostname "); ok {
			hostname := strings.TrimSuffix(after, ";")
			hostname = strings.Trim(hostname, "\"")
			if hostname != "" {
				currentHostname = hostname
			}
		} else if after, ok := strings.CutPrefix(line, "binding state "); ok {
			state := strings.TrimSuffix(after, ";")
